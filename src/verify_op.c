...
extern void COMPUTE_NAME_REF(int m0, int k0, float *input_distributed,
                             float *weights_distributed,
                             float *output_distributed);

extern void COMPUTE_NAME_TST(int m0, int k0, float *input_distributed,
                             float *weights_distributed,
                             float *output_distributed);
...
int main(int argc, char *argv[]) {
  ...
  for (int p = min_size; p < max_size; p += step_size) {
    ...
    float *input_sequential_ref =
        (float *)malloc(sizeof(float) * input_sequential_sz);
    float *output_sequential_ref =
        (float *)malloc(sizeof(float) * output_sequential_sz);
    float *weights_sequential_ref =
        (float *)malloc(sizeof(float) * weights_sequential_sz);

    float *input_sequential_tst =
        (float *)malloc(sizeof(float) * input_sequential_sz);
    float *output_sequential_tst =
        (float *)malloc(sizeof(float) * output_sequential_sz);
    float *weights_sequential_tst =
        (float *)malloc(sizeof(float) * weights_sequential_sz);
    ...
    if (rid == 0) { /* root node */
      ...
      memcpy(input_sequential_tst, input_sequential_ref,
             input_sequential_sz * sizeof(float));
  ...
  Run the reference
    */
    ...
    DISTRIBUTED_ALLOCATE_NAME_REF(m0, k0, &input_distributed_ref,
                                  &weights_distributed_ref,
                                  &output_distributed_ref);

    // Distribute the sequential buffers
    DISTRIBUTE_DATA_NAME_REF(m0, k0, input_sequential_ref,
                             weights_sequential_ref, input_distributed_ref,
                             weights_distributed_ref);
    ...
    COLLECT_DATA_NAME_REF(m0, k0, output_distributed_ref,
                          output_sequential_ref);

    // Finally free the buffers
    DISTRIBUTED_FREE_NAME_REF(m0, k0, input_distributed_ref,
                              weights_distributed_ref, output_distributed_ref);
    ...
    // Allocate distributed buffers for the reference
    DISTRIBUTED_ALLOCATE_NAME_TST(m0, k0, &input_distributed_tst,
                                  &weights_distributed_tst,
                                  &output_distributed_tst);

    // Distribute the sequential buffers
    DISTRIBUTE_DATA_NAME_TST(m0, k0, input_sequential_tst,
                             weights_sequential_tst, input_distributed_tst,
                             weights_distributed_tst);
    ...
    // Collect the distributed data and write it to a sequential buffer
    COLLECT_DATA_NAME_TST(m0, k0, output_distributed_tst,
                          output_sequential_tst);

    // Finally free the buffers
    DISTRIBUTED_FREE_NAME_TST(m0, k0, input_distributed_tst,
                              weights_distributed_tst, output_distributed_tst);

    // We only need to verify the results sequentially
    if (rid == 0) {
      /* root node */

      float res = max_pair_wise_diff(m0, 1, 1, 1, output_sequential_ref,
                                     output_sequential_tst);
