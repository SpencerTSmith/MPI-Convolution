...
extern void COMPUTE_NAME_REF(int m0, int k0, float *input_distributed, float *weights_distributed,
                             float *output_distributed);

extern void COMPUTE_NAME_TST(int m0, int k0, float *input_distributed, float *weights_distributed,
                             float *output_distributed);

extern void DISTRIBUTED_ALLOCATE_NAME_REF(int m0, int k0, float **input_distributed,
                                          float **weights_distributed, float **output_distributed);

extern void DISTRIBUTED_ALLOCATE_NAME_TST(int m0, int k0, float **input_distributed,
                                          float **weights_distributed, float **output_distributed);

extern void DISTRIBUTE_DATA_NAME_REF(int m0, int k0, float *input_sequential,
                                     float *weights_sequential, float *input_distributed,
                                     float *weights_distributed);

extern void DISTRIBUTE_DATA_NAME_TST(int m0, int k0, float *input_sequential,
                                     float *weights_sequential, float *input_distributed,
                                     float *weights_distributed);

extern void COLLECT_DATA_NAME_REF(int m0, int k0, float *output_distributed,
                                  float *output_sequential);

extern void COLLECT_DATA_NAME_TST(int m0, int k0, float *output_distributed,
                                  float *output_sequential);

extern void DISTRIBUTED_FREE_NAME_REF(int m0, int k0, float *input_distributed,
                                      float *weights_distributed, float *output_distributed);

extern void DISTRIBUTED_FREE_NAME_TST(int m0, int k0, float *input_distributed,
                                      float *weights_distributed, float *output_distributed);
...
int main(int argc, char *argv[]) {
    ...
    for (int p = min_size; p < max_size; p += step_size) {
        ...
        if (rid == 0) { /* root node */
            ...
            memcpy(weights_sequential_tst, weights_sequential_ref,
                   weights_sequential_sz * sizeof(float));
            memcpy(output_sequential_tst, output_sequential_ref,
                   output_sequential_sz * sizeof(float));
        } else { /* all other nodes. */
        }
      ...
      Run the reference
        */
        ...
        DISTRIBUTED_ALLOCATE_NAME_REF(m0, k0, &input_distributed_ref, &weights_distributed_ref,
                                      &output_distributed_ref);

        // Distribute the sequential buffers
        DISTRIBUTE_DATA_NAME_REF(m0, k0, input_sequential_ref, weights_sequential_ref,
                                 input_distributed_ref, weights_distributed_ref);

        // Perform the computation
        COMPUTE_NAME_REF(m0, k0, input_distributed_ref, weights_distributed_ref,
                         output_distributed_ref);

        // Collect the distributed data and write it to a sequential buffer
        COLLECT_DATA_NAME_REF(m0, k0, output_distributed_ref, output_sequential_ref);

        // Finally free the buffers
        DISTRIBUTED_FREE_NAME_REF(m0, k0, input_distributed_ref, weights_distributed_ref,
                                  output_distributed_ref);
        ...
        // Allocate distributed buffers for the reference
        DISTRIBUTED_ALLOCATE_NAME_TST(m0, k0, &input_distributed_tst, &weights_distributed_tst,
                                      &output_distributed_tst);

        // Distribute the sequential buffers
        DISTRIBUTE_DATA_NAME_TST(m0, k0, input_sequential_tst, weights_sequential_tst,
                                 input_distributed_tst, weights_distributed_tst);

        // Perform the computation
        COMPUTE_NAME_TST(m0, k0, input_distributed_tst, weights_distributed_tst,
                         output_distributed_tst);

        // Collect the distributed data and write it to a sequential buffer
        COLLECT_DATA_NAME_TST(m0, k0, output_distributed_tst, output_sequential_tst);

        // Finally free the buffers
        DISTRIBUTED_FREE_NAME_TST(m0, k0, input_distributed_tst, weights_distributed_tst,
                                  output_distributed_tst);
        ...
        if (rid == 0) {
            ...
            float res =
                max_pair_wise_diff(m0, 1, 1, 1, output_sequential_ref, output_sequential_tst);

            fprintf(result_file, "%i,%i,%i,", num_ranks, m0, k0);
            ...
            if (res > ERROR_THRESHOLD) {
                ...
                for (int i = 0; i < m0; i++) {
                    if (output_sequential_tst[i] != output_sequential_ref[i])
                        printf("index = %d ref = %.9f test = %.9f\n", i, output_sequential_ref[i],
                               output_sequential_tst[i]);
                }
            } else
