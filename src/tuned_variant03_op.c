...

{
  ...
  */
  ...
  if (rid == root_rid) {
    ...
    for (int i0 = 0; i0 <= before_wrap; i0 += 8) {
      __m256 input_reg = _mm256_loadu_ps(&input_distributed[i0]);
      __m256 res_reg = _mm256_setzero_ps();
      for (int j = 0; j < k0; j++) {
        __m256 current_weight_reg =
            _mm256_broadcast_ss(&weights_distributed[j]);
        __m256 res_reg =
            _mm256_fmadd_ps(input_reg, current_weight_reg, res_reg);
        // Rotate input register
        input_reg = _mm256_permutevar8x32_ps(input_reg, rotate_indices);
      }
